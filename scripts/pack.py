...
def pack_files(source_dir, output_file):
    ...
    all_files = sorted(swift_files + plist_files + entitlements_files)

    if not all_files:
        print(f"No .swift, .plist, or .entitlements files found in {source_dir}")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pack .swift, .plist, and .entitlements files into a single file.",
